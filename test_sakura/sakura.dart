...
class Sakura
{
    ...
    void grows( NatureSpirit _ , List<int> i ) {
        int isMine = i[1];
        int bill = _.agent[isMine].getCost(this.size+1);
    ...
    void completes( NatureSpirit _ , List<int> i ) {
        ...
        _.agent[isMine].fuzzy += kFUZZY[ kC ] + mapFields[ this.cellIndex ].richness - 4;
    ...
    for( int indexStack in g.map.values ) {
        ...
        if( t.size < 3 ) {
            ...
            if( 4 <= g.agent[t.isMine].sun ) {
                int K   = g.agent[ t.isMine ].fuzzy + kFUZZY[ kC] + mapFields[t.cellIndex].richness - 4;
...
class Tree {
    ...
    late int i1;                                            //  WIN     parent?.w1[i1]++; parent?.n1[i1]++;
                                                            //  LOOSE   parent?.n1[i1]++;
    List<int> w1 = [ 0 , 0 , 0 ] , n1 = [ 1 , 1 , 1 ];
    ...
    void updateHeap( SplayTreeMap t )
    {
        ...
        int b = - ( 3 * this.game.agent[kM].fuzzy + this.game.agent[kO].fuzzy );
    ...
    void updateScoring( SplayTreeMap t )
    {
        ...
        scoring = - ( 3 * score[kM] + score[kO] );

        //error("updateScoring $score ${score[kM] >= score[kO]} ${this.game.day}");
        //if( score[kM] >= score[kO] ) {
        {   Tree _ = this;
            while( _.parent != null ) {
                _.parent!.w1 [ _.i1 ]+= score[kM];
                _.parent!.n1 [ _.i1 ]+= score[kO];
                _ = _.parent!; } }
        //else {
        //    Tree _ = this;
        //    while( _.parent != null ) {
        ...
        //error("END >>> updateScoring");

        //  Add in heap
        while( t.containsKey(scoring) ) scoring--;
        t[scoring] = this;
    }
...
void resolveNode( int end , Tree g , SplayTreeMap bst , SplayTreeMap brt )
{
    ...
    for( int m1 = 0 ; m1 < 3 ; m1++ ) {
        ...
        for( int o1 = 0 ; o1 < 3 ; o1++ ) {
            ...
            child =
            new Tree( g : node.game)..i1 = m1..mine = mine..opp = opp..parent = node ;
...
void main()
{
    ...
    while (true)
    {
        ...
        int bestIndex = 0;
        for( int i = 0 ; i < 3 ; i++ ) {
            if( node.w1[i] / node.n1[i] > bestWin ) {
                bestIndex = i;
                bestWin = node.w1[i] / node.n1[i]; } }
        chosen = node.child[bestIndex ][0]!;
